...
* [ ] **1.0 Setup:** Initialize FastAPI, SQLAlchemy (Async), Jinja2, HTMX.
* [ ] **1.1 Component Model:** Create `Component` model with a JSON `stats` field (flexible for "Fuel" vs "Antimatter").
* [ ] **1.2 Component Seeder:** Create a script to populate the DB with 10 basic items (Engine, Rifle, Radio, Rations) so we have test data.

### Phase 2: The Asset Designer (The "Physical" Layer)

* [ ] **2.0 Asset Schema:** Create `Asset` and `AssetComponent` models.
* [ ] **2.1 Designer UI:** Create `asset_designer.html`.
...
* [ ] **2.2 Validation Logic:** Write a Python service `validate_asset(asset_id)` that returns `valid: boolean` and `violations: list` (e.g., "Power draw exceeds generator output").

### Phase 3: The Unit Designer (The "Strategic" Layer)

* [ ] **3.0 Unit Schema:** Create the recursive `Unit` model.
* [ ] **3.1 Tree View UI:** Create `unit_designer.html`. Use a recursive Jinja template or a JS tree library to visualize the hierarchy.
* [ ] **3.2 Drag-and-Drop API:** Implement endpoints to `move_unit(child_id, new_parent_id)`.
...
* [ ] **3.4 Capability Tags:** Implement logic to aggregate tags. (e.g., If 3 sub-units have "Cyber", the Parent Unit gets "Cyber Support: High").

### Phase 4: Simulation Integration (The "Test")

* [ ] **4.0 Scenario Definition:** Update to accept full **Unit Trees** instead of just lists of assets.
* [ ] **4.1 Logic Update:**
